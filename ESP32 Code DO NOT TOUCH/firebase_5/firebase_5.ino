...
void setup() {
  ...
  // Generate unique device ID (shorter for display)
  uint64_t chipid = ESP.getEfuseMac();
  deviceId = "HSG" + String((uint32_t)(chipid & 0xFFFFFF), HEX);
...
void connectToSavedWiFi() {
  ...
  display.println(ssid.substring(0, 20));
  display.display();
...
void displaySetupMode() {
  ...
  String shortSSID = apSSID;
  if (shortSSID.length() > 20) {
    shortSSID = shortSSID.substring(0, 17) + "...";
  }
...
void displayOff() {
  ...
  if (readingTaken) {
    ...
    display.println(" o/oo");  // Using o/oo for per mille
  } else {
