...
<body>
    ...
    <script>
        ...
        function drawFriendChart(friend) {
            ...
            if (!canvas || !friend.history) return;
        ...
        function messageFriend(friendName) {
            const messages = [
                ...
                "Having fun? Stay safe! 🎊",
                "Water break time! 💧"
            ];
            const randomMsg = messages[Math.floor(Math.random() * messages.length)];
            showNotification(`📱 Sent to ${friendName.split(' ')[0]}: "${randomMsg}"`);
            closeModal();
        }
        
        function callFriend(friendName) {
            showNotification(`📞 Calling ${friendName.split(' ')[0]}...`);
            // In real app, would initiate call
            closeModal();
        }
        
        function shareLocation(friendName) {
            showNotification(`📍 Location shared with ${friendName.split(' ')[0]}`);
            closeModal();
        }
        
        function selectBuddy(buddyName) {
            localStorage.setItem('buddy', buddyName);
            showNotification(`👥 ${buddyName.split(' ')[0]} is now your buddy!`);
        ...
        function callSoberFriend() {
            ...
            if (soberFriends.length > 0) {
                ...
                showNotification(`📞 Calling ${friend.name.split(' ')[0]}...`);
            } else {
        ...
        function initializeVoiceCommands() {
            if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
                ...
                voiceRecognition.onerror = function() {
                    showNotification('🎤 Voice command failed. Try again!');
                    document.querySelector('.voice-indicator').classList.remove('listening');
                };
        ...
        function updateDrinkStats() {
            ...
            const recentDrinks = drinkHistory.filter(d => d.time > oneHourAgo && d.type !== 'water').length;
            
            // Update display
            document.getElementById('totalDrinks').textContent = totalDrinks;
            document.getElementById('totalWater').textContent = totalWater;
            document.getElementById('totalAlcohol').textContent = totalAlcohol.toFixed(0) + 'ml';
            document.getElementById('drinkRate').textContent = recentDrinks + '/hr';
        }
        ...
        function showEmergencyReport() {
            const report = {
                ...
                totalAlcohol: drinkHistory.reduce((sum, d) => sum + parseFloat(d.pureAlcohol), 0),
                userData: {
                    name: currentUser,
...
Emergency Contact: ${report.userData.emergencyContact || 'Not provided'}
...
- Consider IV fluids if dehydrated
            ...
            const modalContent = `
                ...
                <div style="display: flex; gap: 10px; justify-content: center;">
                    <button class="btn btn-primary" onclick="copyEmergencyReport('${btoa(reportText)}')">
        ...
        function copyEmergencyReport(encodedReport) {
        ...
        function playTrack(index) {
            ...
            if (playlist[index].playing) {
                showNotification(`🎵 Now playing: ${playlist[index].title}`);
            }
        }
        ...
        function createPredictionEngine() {
            ...
            const predictionHtml = `
                <div class="prediction-card">
                    <h3>🔮 Party Predictions</h3>
                    <div class="prediction-value">Peak BAC: ${(avgBac * 1.5).toFixed(3)}‰</div>
        ...
        function advancedInit() {
            ...
            // Welcome message
            setTimeout(() => {
                showNotification('🎉 Welcome to HSG Party Tracker! Stay safe and have fun!');
            }, 1000);
        ...
        window.addEventListener('unhandledrejection', event => {
            ...
            showNotification('⚠️ Something went wrong. Try refreshing.');
        });
